...
check_dependency() {
    ...
    if ! pip show "$package" 2>/dev/null | grep -q "Version: $version"; then
        log "ERROR - $package==$version required but not found or incorrect version"
        ERROR_FOUND=1
    else
        log "INFO - $package==$version found"
    fi
}

check_dependency tenacity 8.5.0
check_dependency pymongo 4.10.1
check_dependency boto3 1.35.39
check_dependency confluent-kafka 2.5.3
check_dependency pytest 8.3.3
check_dependency pytest-mock 3.14.0
check_dependency avro-python3 1.10.2 
