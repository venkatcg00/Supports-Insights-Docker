...
def make_dag_run_entry(airflow_dag_run_id: str, source: str, dag_run_status: str) -> int:
    with pg_hook.get_conn() as conn:
        try:
            with conn.cursor() as cur:
...
def end_dag_run(
    dag_run_id: int, batch_count: int,
    insert_count: int, update_count: int, duplicate_count: int,
    valid_count: int, source_checkpoint: int
) -> bool:
    with pg_hook.get_conn() as conn:
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT run_start_date FROM aud.dag_runs WHERE dag_run_id = %s", (dag_run_id,))
                ...
                validity_percentage = round((valid_count / batch_count) * 100, 2) if batch_count > 0 else 0.0
                ...
                cur.execute("""
                    ...
                    WHERE dag_run_id = %s
                """, (
                    run_end_time, dag_run_status, batch_count,
                    insert_count, update_count, duplicate_count,
                    valid_count, validity_percentage, run_duration,
                    source_checkpoint, dag_run_id
                ))
                conn.commit()
                return True
        except Exception:
...
def fetch_lookup_dictionary(dict_type: str, source: str) -> Optional[dict[int, str]]:
    queries = {
        'AGENT_DICT': """
...
def create_temp_table(schema_name: str, table_name: str, col_name: str, data_type: str):
    pg_hook.run(f"""
...
def get_data_counts(src_table: str, prs_table: str, src_id_column: str, prs_id_column: str):
    counts_query = f"""
        SELECT
