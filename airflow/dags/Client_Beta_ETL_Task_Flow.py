    ...
    try:
        spark = (
            SparkSession.builder.appName(app_name).master("local[*]").config(
                ...
                "8").config("spark.default.parallelism", "8").config(
                    "spark.jars",
                    "/opt/spark/jars/postgresql-jdbc.jar").getOrCreate())
...
def handle_task_error(task_name: str,
                      error: Exception,
                      dag_run_id: str = None):
...
def client_beta_etl_dag():
    ...
    with DAG(
        ...
        client_beta_branch_b_end = EmptyOperator(
            task_id="client_beta_branch_b_end")
        client_beta_branch_a_start = EmptyOperator(
            task_id="client_beta_branch_a_start")
        client_beta_end = EmptyOperator(task_id="client_beta_end",
        ...
        def client_beta_check_new_data(ti=None):
            try:
                ...
                logger.info(
                    f"Checkpoint={checkpoint}, Latest={high_offset}, New={signal}"
                )
                return True
            except Exception as e:
        ...
        def client_beta_branch_func(ti):
            signal = ti.xcom_pull(task_ids="client_beta_check_new_data",
                                  key="signal")
            if signal not in ["Y", "N"]:
                raise AirflowException(f"Invalid signal value: {signal}")
            logger.info(
                f"Branching decision: {'client_beta_branch_a_start' if signal == 'Y' else 'client_beta_branch_b_end'}"
            )
            return ("client_beta_branch_a_start"
                    if signal == "Y" else "client_beta_branch_b_end")

        client_beta_branch = BranchPythonOperator(
            task_id="client_beta_branch",
            python_callable=client_beta_branch_func)

        @task(task_id="client_beta_source_to_lnd")
        def client_beta_source_to_lnd(ti=None):
            airflow_dag_run_id = f"{ti.dag_id} - {ti.run_id}"
            checkpoint = ti.xcom_pull(task_ids="client_beta_check_new_data",
                                      key="checkpoint")
            dag_run_id = make_dag_run_entry(airflow_dag_run_id, SOURCE,
                                            "STARTED")
            ...
            try:
                logger.info(
                    f"Fetching Kafka messages from offset > {checkpoint} for topic '{topic}'"
                )
                start_time = time()
                max_wait = 10  # seconds
                ...
                while True:
                    ...
                    try:
                        ...
                        new_records.append({
                            "support_identifier":
                            safe_text(root, "SUPPORT_IDENTIFIER"),
                            "contact_regarding":
                            safe_text(root, "CONTACT_REGARDING"),
                            "agent_code":
                            safe_text(root, "AGENT_CODE"),
                            "date_of_interaction":
                            safe_text(root, "DATE_OF_INTERACTION"),
                            "status_of_interaction":
                            safe_text(root, "STATUS_OF_INTERACTION"),
                            "type_of_interaction":
                            safe_text(root, "TYPE_OF_INTERACTION"),
                            "customer_type":
                            safe_text(root, "CUSTOMER_TYPE"),
                            "contact_duration":
                            safe_text(root, "CONTACT_DURATION"),
                            "after_contact_work_time":
                            safe_text(root, "AFTER_CONTACT_WORK_TIME"),
                            "incident_status":
                            safe_text(root, "INCIDENT_STATUS"),
                            "first_contact_solve":
                            safe_text(root, "FIRST_CONTACT_SOLVE"),
                            "type_of_resolution":
                            safe_text(root, "TYPE_OF_RESOLUTION"),
                            "support_rating":
                            safe_text(root, "SUPPORT_RATING"),
                            "time_stamp":
                            safe_text(root, "TIME_STAMP"),
                            "kafka_offset":
                            msg.offset(),
                        })
                    except Exception as e:
            ...
            try:
                ...
                df = (df.withColumn(
                    ...
                    "row_num",
                    row_number().over(
                        Window.partitionBy("support_identifier").orderBy(
                            col("time_stamp").cast("timestamp").desc())),
                ).filter(col("row_num") == 1).drop("row_num").select(
                    col("support_identifier").cast("integer"),
                    *hash_columns[1:],
                    "hash_key",
                    "kafka_offset",
                ))
                ...
                df.drop("kafka_offset").write.option(
                    "truncate",
                    "true").format("jdbc").option("url", jdbc_url).option(
                        "dbtable",
                        "lnd.client_beta_cs_data").option("user", user).option(
                            "password",
                            password).option("driver",
                                             "org.postgresql.Driver").mode(
                                                 "overwrite").save()
        ...
        def client_beta_lnd_to_prs(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_beta_source_to_lnd",
                                      key="dag_run_id")
            ...
            try:
                ...
                """
                df = (spark.read.format("jdbc").option("url", jdbc_url).option(
                    "dbtable",
                    f"({query}) as src").option("user", user).option(
                        "password",
                        password).option("driver",
                                         "org.postgresql.Driver").load())

                insert_count, update_count, duplicate_count = get_data_counts(
                    "lnd.client_beta_cs_data",
                    "prs.client_beta_cs_data",
                    "support_identifier",
                    "support_identifier",
                )
                ...
                df_filtered = df.filter(
                    col("merge_type").isin("INSERT", "UPDATE"))
                create_temp_table("prs", "client_beta_temp", "prs_record_id",
                                  "INT")
                df_filtered.filter(
                    col("merge_type") == "UPDATE"
                ).select("prs_record_id").write.format("jdbc").option(
                    "url",
                    jdbc_url).option("dbtable", "prs.client_beta_temp").option(
                        "user", user).option("password", password).option(
                            "driver",
                            "org.postgresql.Driver").mode("append").save()
                ...
                df_filtered.select(
                    lit(source_id).alias("source_id"),
                    lit(dag_run_id).alias("dag_run_id"),
                    concat_ws("-", lit(SOURCE),
                              col("support_identifier")).alias(
                                  "source_system_identifier"),
                    col("support_identifier"),
                    col("contact_regarding"),
                    col("agent_code"),
                    col("date_of_interaction"),
                    col("status_of_interaction"),
                    col("type_of_interaction"),
                    col("customer_type"),
                    col("contact_duration"),
                    col("after_contact_work_time"),
                    col("incident_status"),
                    col("first_contact_solve"),
                    col("type_of_resolution"),
                    col("support_rating"),
                    col("lnd_hash_key").alias("hash_key"),
                ).write.format("jdbc").option("url", jdbc_url).option(
                    "dbtable", "prs.client_beta_cs_data").option(
                        "user", user).option("password", password).option(
                            "driver",
                            "org.postgresql.Driver").mode("append").save()
        ...
        def client_beta_prs_to_cdc(ti=None):
            ...
            try:
                ...
                df.write.option("truncate", "true").format("jdbc").option(
                    "url", jdbc_url).option(
                        "dbtable", "cdc.client_beta_cs_data").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
        ...
        def client_beta_cdc_to_predm(ti=None):
            ...
            try:
                ...
                df = (df.withColumn(
                    "handle_time_parts", split(col("contact_duration"), ":")
                ).withColumn(
                    "handle_time_int",
                    col("handle_time_parts")[0].cast("int") * 3600 +
                    col("handle_time_parts")[1].cast("int") * 60 +
                    col("handle_time_parts")[2].cast("int"),
                ).drop("handle_time_parts").withColumnRenamed(
                    "handle_time_int", "handle_time").withColumn(
                        "work_time_parts",
                        split(col("after_contact_work_time"), ":")).withColumn(
                            "work_time_int",
                            col("work_time_parts")[0].cast("int") * 3600 +
                            col("work_time_parts")[1].cast("int") * 60 +
                            col("work_time_parts")[2].cast("int"),
                        ).drop("work_time_parts").withColumnRenamed(
                            "work_time_int", "work_time"))
                ...
                transformed_df = (df.join(
                    spark.createDataFrame(
                        ...
                        ["agent_id", "pseudo_code"],
                    ),
                    df["agent_code"] == col("pseudo_code"),
                    "left",
                ).drop("pseudo_code").join(
                    spark.createDataFrame(
                        [(k, v) for k, v in support_dict.items()],
                        ["support_area_id", "support_area_name"],
                    ),
                    df["contact_regarding"] == col("support_area_name"),
                    "left",
                ).drop("support_area_name").join(
                    spark.createDataFrame(
                        [(k, v) for k, v in cust_dict.items()],
                        ["customer_type_id", "customer_type_name"],
                    ),
                    df["customer_type"] == col("customer_type_name"),
                    "left",
                ).drop("customer_type_name").select(
                    col("source_id"),
                    col("source_record_id"),
                    col("source_system_identifier"),
                    col("agent_id"),
                    col("date_of_interaction").alias("interaction_date"),
                    col("support_area_id"),
                    col("status_of_interaction").alias("interaction_status"),
                    col("type_of_interaction").alias("interaction_type"),
                    col("customer_type_id"),
                    col("handle_time"),
                    col("work_time"),
                    col("first_contact_solve").alias(
                        "first_contact_resolution"),
                    col("incident_status").alias("query_status"),
                    col("type_of_resolution").alias("solution_type"),
                    col("support_rating").alias("customer_rating"),
                    col("dag_run_id"),
                ))

                valid_check_df = transformed_df.withColumn(
                    "is_valid",
                    when(
                        col("agent_id").isNull()
                ...
                valid_check_df.write.option(
                    "truncate",
                    "true").format("jdbc").option("url", jdbc_url).option(
                        "dbtable",
                        "pre_dm.customer_support_stage_beta").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()

                invalid_records = valid_check_df.filter(
                    col("is_valid") == False).select(
                        col("dag_run_id"),
                        col("source_id"),
                        lit("INVALID").alias("error_type"),
                        lit("A REQUIRED COLUMN IS NULL").alias(
                            "error_description"),
                        col("source_system_identifier"),
                        col("source_record_id"),
                    )

                create_temp_table("aud", "data_error_temp_beta",
                                  "source_system_identifier", "TEXT")
                invalid_records.select(
                    "source_system_identifier").write.format("jdbc").option(
                        "url", jdbc_url).option(
                            "dbtable", "aud.data_error_temp_beta").option(
                                "user",
                                user).option("password", password).option(
                                    "driver", "org.postgresql.Driver").mode(
                                        "append").save()
                ...
                invalid_records.write.format("jdbc").option(
                    "url", jdbc_url).option(
                        "dbtable", "aud.data_error_history").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
        ...
        def client_beta_predm_to_dm(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_beta_source_to_lnd",
                                      key="dag_run_id")
            source_id = ti.xcom_pull(task_ids="client_beta_lnd_to_prs",
                                     key="source_id")
            ...
            try:
                ...
                create_temp_table("dm", "temp_table_cb",
                                  "source_system_identifier", "TEXT")
                df.filter(col("merge_type") == "UPDATE").select(
                    "source_system_identifier").write.format("jdbc").option(
                        "url",
                        jdbc_url).option("dbtable", "dm.temp_table_cb").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
                ...
                pg_hook.run("""
                    ...
                    SET is_active = FALSE, end_date = CURRENT_TIMESTAMP
                    FROM dm.temp_table_cb AS tmp
                    WHERE main.is_active = TRUE AND main.source_system_identifier = tmp.source_system_identifier
                """)
                drop_temp_table("dm", "temp_table_cb")
                ...
                df.select(
                    ...
                    "dbtable", "dm.customer_support_fact").option(
                        "user", user).option("password", password).option(
                            "driver",
                            "org.postgresql.Driver").mode("append").save()

                update_dag_run_status(
                    dag_run_id, "PROCESSING - ALL LOADING STAGES COMPLETED")
                return True
            except Exception as e:
        ...
        def client_beta_finalize_dag_run(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_beta_source_to_lnd",
                                      key="dag_run_id")
            try:
                batch_count = (ti.xcom_pull(
                    task_ids="client_beta_source_to_lnd", key="batch_count")
                               or 0)
                insert_count = (ti.xcom_pull(task_ids="client_beta_lnd_to_prs",
                                             key="insert_count") or 0)
                update_count = (ti.xcom_pull(task_ids="client_beta_lnd_to_prs",
                                             key="update_count") or 0)
                duplicate_count = (ti.xcom_pull(
                    task_ids="client_beta_lnd_to_prs", key="duplicate_count")
                                   or 0)
                source_id = ti.xcom_pull(task_ids="client_beta_lnd_to_prs",
                                         key="source_id")
                valid_count = get_valid_count(source_id, dag_run_id) or 0
                new_checkpoint = (ti.xcom_pull(
                    task_ids="client_beta_source_to_lnd", key="new_checkpoint")
                                  or 0)
        ...
        (client_beta_branch >> client_beta_branch_a_start >> client_beta_lnd >>
         client_beta_prs >> client_beta_cdc >> client_beta_predm >>
         client_beta_dm >> client_beta_finalize >> client_beta_end)
