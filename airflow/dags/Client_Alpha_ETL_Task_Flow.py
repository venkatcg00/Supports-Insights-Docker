...
def client_alpha_etl_dag():
    ...
    with DAG(
        ...
        client_alpha_branch_b_end = EmptyOperator(
            task_id="client_alpha_branch_b_end")
        client_alpha_branch_a_start = EmptyOperator(
            task_id="client_alpha_branch_a_start")
        client_alpha_end = EmptyOperator(task_id="client_alpha_end",
        ...
        def client_alpha_check_new_data(ti=None):
            try:
                ...
                with MongoHook(
                        mongo_conn_id="mongo_project_connection") as mongo:
                    ...
                    logger.info(
                        f"New data check result: {'Found' if new_data else 'Not found'}"
                    )
                return True
            except Exception as e:
        ...
        def client_alpha_branch_func(ti):
            signal = ti.xcom_pull(task_ids="client_alpha_check_new_data",
                                  key="signal")
            if signal not in ["Y", "N"]:
                raise AirflowException(f"Invalid signal value: {signal}")
            logger.info(
                f"Branching decision: {'client_alpha_branch_a_start' if signal == 'Y' else 'client_alpha_branch_b_end'}"
            )
            return ("client_alpha_branch_a_start"
                    if signal == "Y" else "client_alpha_branch_b_end")

        client_alpha_branch = BranchPythonOperator(
            task_id="client_alpha_branch",
            python_callable=client_alpha_branch_func)

        @task(task_id="client_alpha_source_to_lnd")
        def client_alpha_source_to_lnd(ti=None):
            airflow_dag_run_id = f"{ti.dag_id} - {ti.run_id}"
            checkpoint = ti.xcom_pull(task_ids="client_alpha_check_new_data",
                                      key="checkpoint")
            ...
            try:
                ...
                df = (df.withColumnRenamed(
                    "serial_number", "serial_number").withColumnRenamed(
                        "INTERACTION_ID", "interaction_id").withColumnRenamed(
                            "SUPPORT_CATEGORY",
                            "support_category").withColumnRenamed(
                                "AGENT_PSEUDO_NAME",
                                "agent_pseudo_name").withColumnRenamed(
                                    "CONTACT_DATE",
                                    "contact_date").withColumnRenamed(
                                        "INTERACTION_STATUS",
                                        "interaction_status").
                      withColumnRenamed(
                          "INTERACTION_TYPE",
                          "interaction_type").withColumnRenamed(
                              "TYPE_OF_CUSTOMER",
                              "type_of_customer").withColumnRenamed(
                                  "INTERACTION_DURATION",
                                  "interaction_duration").withColumnRenamed(
                                      "TOTAL_TIME", "total_time").
                      withColumnRenamed(
                          "STATUS_OF_CUSTOMER_INCIDENT",
                          "status_of_customer_incident").withColumnRenamed(
                              "RESOLVED_IN_FIRST_CONTACT",
                              "resolved_in_first_contact").withColumnRenamed(
                                  "SOLUTION_TYPE",
                                  "solution_type").withColumnRenamed(
                                      "RATING", "rating").
                      withColumn(
                          "row_num",
                          row_number().over(
                              Window.partitionBy("interaction_id").orderBy(
                                  col("serial_number").desc())),
                ...
                df.write.option("truncate", "true").format("jdbc").option(
                    "url", jdbc_url).option(
                        "dbtable", "lnd.client_alpha_cs_data").option(
                ...
                new_checkpoint = pg_hook.get_first(
                    "SELECT COALESCE(MAX(serial_number), 0) FROM lnd.client_alpha_cs_data"
                )[0]
        ...
        def client_alpha_lnd_to_prs(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_alpha_source_to_lnd",
                                      key="dag_run_id")
            ...
            try:
                ...
                insert_count, update_count, duplicate_count = get_data_counts(
                    "lnd.client_alpha_cs_data",
                    "prs.client_alpha_cs_data",
                    "interaction_id",
                    "interaction_id",
                )
                ...
                create_temp_table("prs", "client_alpha_temp", "prs_record_id",
                                  "INT")
                df_filtered.filter(col("merge_type") == "UPDATE").select(
                    "prs_record_id").write.format("jdbc").option(
                        "url", jdbc_url).option(
                            "dbtable", "prs.client_alpha_temp").option(
                                "user",
        ...
        def client_alpha_prs_to_cdc(ti=None):
            ...
            try:
                ...
                df.write.option("truncate", "true").format("jdbc").option(
                    "url", jdbc_url).option(
                        "dbtable", "cdc.client_alpha_cs_data").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
        ...
        def client_alpha_cdc_to_predm(ti=None):
            ...
            try:
                ...
                transformed_df = (df.join(
                    ...
                    df["agent_pseudo_name"] == col("pseudo_code"),
                    "left",
                    ...
                    col("agent_id"),
                    col("contact_date").alias("interaction_date"),
                    col("support_area_id"),
                    col("interaction_status"),
                    col("interaction_type"),
                ...
                valid_check_df.write.option(
                    ...
                    "true").format("jdbc").option("url", jdbc_url).option(
                        "dbtable",
                        "pre_dm.customer_support_stage_alpha").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
        ...
        def client_alpha_predm_to_dm(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_alpha_source_to_lnd",
                                      key="dag_run_id")
            source_id = ti.xcom_pull(task_ids="client_alpha_lnd_to_prs",
                                     key="source_id")
            ...
            try:
                ...
                create_temp_table("dm", "temp_table_ca",
                                  "source_system_identifier", "TEXT")
                df.filter(col("merge_type") == "UPDATE").select(
                    "source_system_identifier").write.format("jdbc").option(
                        "url",
                        jdbc_url).option("dbtable", "dm.temp_table_ca").option(
                            "user", user).option("password", password).option(
                                "driver", "org.postgresql.Driver").mode(
                                    "overwrite").save()
        ...
        def client_alpha_finalize_dag_run(ti=None):
            ...
            try:
                batch_count = (ti.xcom_pull(
                    task_ids="client_alpha_source_to_lnd", key="batch_count")
                               or 0)
                insert_count = (ti.xcom_pull(
                    task_ids="client_alpha_lnd_to_prs", key="insert_count")
                                or 0)
                update_count = (ti.xcom_pull(
                    task_ids="client_alpha_lnd_to_prs", key="update_count")
                                or 0)
                duplicate_count = (ti.xcom_pull(
                    task_ids="client_alpha_lnd_to_prs", key="duplicate_count")
                                   or 0)
                source_id = ti.xcom_pull(task_ids="client_alpha_lnd_to_prs",
                                         key="source_id")
                valid_count = get_valid_count(source_id, dag_run_id) or 0
                new_checkpoint = (ti.xcom_pull(
                    task_ids="client_alpha_source_to_lnd",
        ...
        (client_alpha_branch >> client_alpha_branch_a_start >> client_alpha_lnd
         >> client_alpha_prs >> client_alpha_cdc >> client_alpha_predm >>
         client_alpha_dm >> client_alpha_finalize >> client_alpha_end)
