...
def client_gamma_etl_dag():
    ...
    with DAG(
        ...
        client_gamma_branch_b_end = EmptyOperator(
            task_id="client_gamma_branch_b_end")
        client_gamma_branch_a_start = EmptyOperator(
            task_id="client_gamma_branch_a_start")
        client_gamma_end = EmptyOperator(task_id="client_gamma_end",
        ...
        def client_gamma_check_new_data(ti=None):
            try:
                ...
                logger.info(
                    f"New data check result: {'Found' if signal == 'Y' else 'Not found'}"
                )
                return True
            except Exception as e:
        ...
        def client_gamma_branch_func(ti):
            signal = ti.xcom_pull(task_ids="client_gamma_check_new_data",
                                  key="signal")
            if signal not in ["Y", "N"]:
                raise AirflowException(f"Invalid signal value: {signal}")
            logger.info(
                f"Branching decision: {'client_gamma_branch_a_start' if signal == 'Y' else 'client_gamma_branch_b_end'}"
            )
            return ("client_gamma_branch_a_start"
                    if signal == "Y" else "client_gamma_branch_b_end")

        client_gamma_branch = BranchPythonOperator(
            task_id="client_gamma_branch",
            python_callable=client_gamma_branch_func)

        @task(task_id="client_gamma_source_to_lnd")
        def client_gamma_source_to_lnd(ti=None):
            airflow_dag_run_id = f"{ti.dag_id} - {ti.run_id}"
            checkpoint = ti.xcom_pull(task_ids="client_gamma_check_new_data",
                                      key="checkpoint")
            ...
            s3_hook = S3Hook(aws_conn_id="minio_project_connection")
            pattern = r"^\s*(\d{1,10})_client_gamma_support_data_\d{8}_\d{6}\.csv\.gz$"
            ...
            try:
                ...
                for i, (file_index, key) in enumerate(files):
                    ...
                    for col_name in sdf.columns:
                        sdf = sdf.withColumn(
                            col_name,
                            when(col(col_name) == "",
                                 None).otherwise(col(col_name)),
                        )
                    if i != 0:
                        sdf = sdf.toDF(*final_df.columns)
                    final_df = sdf if final_df is None else final_df.unionByName(
                        sdf)

                # Deduplication by ticket_identifier
                final_df = (final_df.withColumn(
                    "row_id", monotonically_increasing_id()).withColumn(
                        "row_num",
                        row_number().over(
                            Window.partitionBy("TICKET_IDENTIFIER").orderBy(
                                "row_id")),
                    ).filter(col("row_num") == 1).drop("row_id", "row_num"))

                df = (final_df.withColumnRenamed(
                    "TICKET_IDENTIFIER",
                    "ticket_identifier").withColumnRenamed(
                        "SUPPORT_CATEGORY",
                        "support_category").withColumnRenamed(
                            "AGENT_NAME", "agent_name").withColumnRenamed(
                                "DATE_OF_CALL",
                                "date_of_call").withColumnRenamed(
                                    "CALL_STATUS", "call_status").
                      withColumnRenamed(
                          "CALL_TYPE", "call_type").withColumnRenamed(
                              "TYPE_OF_CUSTOMER",
                              "type_of_customer").withColumnRenamed(
                                  "DURATION", "duration").withColumnRenamed(
                                      "WORK_TIME",
                                      "work_time").withColumnRenamed(
                                          "TICKET_STATUS",
                                          "ticket_status").withColumnRenamed(
                                              "RESOLVED_IN_FIRST_CONTACT",
                                              "resolved_in_first_contact").
                      withColumnRenamed(
                          "RESOLUTION_CATEGORY",
                          "resolution_category").withColumnRenamed(
                              "RATING", "rating").withColumn(
                                  "hash_key",
                                  md5(
                                      concat_ws(
                                          "||",
                ...
                df.select(
                    col("ticket_identifier").cast("int"),
                    "support_category",
                    "agent_name",
                    "date_of_call",
                    "call_status",
                    ...
                    "rating",
                    "hash_key",
                ).write.option("truncate", "true").format("jdbc").option(
                    "url", jdbc_url).option(
                        "dbtable", "lnd.client_gamma_cs_data").option(
        ...
        def client_gamma_lnd_to_prs(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_gamma_source_to_lnd",
                                      key="dag_run_id")
            ...
            try:
                ...
                insert_count, update_count, duplicate_count = get_data_counts(
                    "lnd.client_gamma_cs_data",
                    "prs.client_gamma_cs_data",
                    "ticket_identifier",
                    "ticket_identifier",
                )
                ...
                create_temp_table("prs", "client_gamma_temp", "prs_record_id",
                                  "INT")
                df_filtered.filter(col("merge_type") == "UPDATE").select(
                    "prs_record_id").write.format("jdbc").option(
                        "url", jdbc_url).option(
                            "dbtable", "prs.client_gamma_temp").option(
                                "user",
                ...
                df_filtered.select(
                    ...
                    concat_ws("-", lit(SOURCE),
                              col("ticket_identifier")).alias(
                                  "source_system_identifier"),
                    col("ticket_identifier"),
                    col("support_category"),
                    col("agent_name"),
                    col("date_of_call"),
                    col("call_status"),
                    col("call_type"),
                    col("type_of_customer"),
                    col("duration"),
                    col("work_time"),
                    col("ticket_status"),
                    col("resolved_in_first_contact"),
                    col("resolution_category"),
                    col("rating"),
                    col("lnd_hash_key").alias("hash_key"),
                ).write.format("jdbc").option("url", jdbc_url).option(
                    "dbtable", "prs.client_gamma_cs_data").option(
                        "user", user).option("password", password).option(
                            "driver",
        ...
        def client_gamma_prs_to_cdc(ti=None):
            ...
            try:
                ...
                df.write.option("truncate", "true").format("jdbc").option(
                    "url", jdbc_url).option(
                        "dbtable", "cdc.client_gamma_cs_data").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
        ...
        def client_gamma_cdc_to_predm(ti=None):
            ...
            try:
                ...
                transformed_df = (df.join(
                    ...
                    df["support_category"] == col("support_area_name"),
                    "left",
                    ...
                    df["type_of_customer"] == col("customer_type_name"),
                    "left",
                    ...
                    col("agent_id"),
                    col("date_of_call").alias("interaction_date"),
                    col("support_area_id"),
                    col("call_status").alias("interaction_status"),
                    col("call_type").alias("interaction_type"),
                    col("customer_type_id"),
                    col("duration").alias("handle_time"),
                    col("work_time"),
                    col("resolved_in_first_contact").alias(
                        "first_contact_resolution"),
                    col("ticket_status").alias("query_status"),
                    col("resolution_category").alias("solution_type"),
                    when(col("rating") == "WORST", 1).when(
                        col("rating") == "BAD",
                        2).when(col("rating") == "NEUTRAL", 3).when(
                            col("rating") == "GOOD", 4).when(
                                col("rating") == "BEST",
                                5).otherwise(None).alias("customer_rating"),
                    col("dag_run_id"),
                ))
                ...
                valid_check_df.write.option(
                    ...
                    "true").format("jdbc").option("url", jdbc_url).option(
                        "dbtable",
                        "pre_dm.customer_support_stage_gamma").option(
                            "user", user).option("password", password).option(
                                "driver",
                                "org.postgresql.Driver").mode("append").save()
        ...
        def client_gamma_predm_to_dm(ti=None):
            dag_run_id = ti.xcom_pull(task_ids="client_gamma_source_to_lnd",
                                      key="dag_run_id")
            source_id = ti.xcom_pull(task_ids="client_gamma_lnd_to_prs",
                                     key="source_id")
            ...
            try:
                ...
                create_temp_table("dm", "temp_table_cg",
                                  "source_system_identifier", "TEXT")
                df.filter(col("merge_type") == "UPDATE").select(
                    "source_system_identifier").write.format("jdbc").option(
                        "url",
                        jdbc_url).option("dbtable", "dm.temp_table_cg").option(
                            "user", user).option("password", password).option(
                                "driver", "org.postgresql.Driver").mode(
                                    "overwrite").save()
                ...
                pg_hook.run("""
                    ...
                    SET is_active = FALSE, end_date = CURRENT_TIMESTAMP
                    FROM dm.temp_table_cg AS tmp
                    WHERE main.is_active = TRUE AND main.source_system_identifier = tmp.source_system_identifier
                """)
                drop_temp_table("dm", "temp_table_cg")
        ...
        def client_gamma_finalize_dag_run(ti=None):
            ...
            try:
                batch_count = (ti.xcom_pull(
                    task_ids="client_gamma_source_to_lnd", key="batch_count")
                               or 0)
                insert_count = (ti.xcom_pull(
                    task_ids="client_gamma_lnd_to_prs", key="insert_count")
                                or 0)
                update_count = (ti.xcom_pull(
                    task_ids="client_gamma_lnd_to_prs", key="update_count")
                                or 0)
                duplicate_count = (ti.xcom_pull(
                    task_ids="client_gamma_lnd_to_prs", key="duplicate_count")
                                   or 0)
                source_id = ti.xcom_pull(task_ids="client_gamma_lnd_to_prs",
                                         key="source_id")
                valid_count = get_valid_count(source_id, dag_run_id) or 0
                new_checkpoint = (ti.xcom_pull(
                    task_ids="client_gamma_source_to_lnd",
                    key="new_checkpoint") or 0)
        ...
        (client_gamma_branch >> client_gamma_branch_a_start >> client_gamma_lnd
         >> client_gamma_prs >> client_gamma_cdc >> client_gamma_predm >>
         client_gamma_dm >> client_gamma_finalize >> client_gamma_end)
